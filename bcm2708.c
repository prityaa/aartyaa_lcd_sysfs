...
int __init bcm_register_device(struct platform_device *pdev)
{
	...
	pr_debug("bcm_register_device : aartyaa registering device = %s\n", 
			pdev->name);
...
static struct resource led_blink_resources[] = {
	[0] = {			/* general purpose I/O */
	       .start = GPIO_BASE,
	       .end = GPIO_BASE + SZ_4K - 1,
	       .flags = IORESOURCE_MEM,
...
static struct gpio_number led_gpio = 
{
	.gpio_num = 18,
};
...
static struct platform_device led_blink_device = 
{
	.name = "led_blink",
	.resource = led_blink_resources,
	.id  = -1,
	.num_resources = 1,
	.dev = 
	{
		.dma_mask = &gpio_dmamask,
		.platform_data = &led_gpio,	
	},
};
...
#ifdef CONFIG_BCM2708_GPIO
	printk("registering the gpio driver \n");
